...
El incremento HTTP cerrado de Dashboard Summary expone `GET /api/dashboard/summary`, obtiene el hogar mediante el contexto HTTP controlado, acepta únicamente los filtros `from` y `to` y delega el cálculo en `getDashboard` sin duplicar lógica financiera.
...
El incremento cerrado `PATCH /api/expenses/{id}` adapta la actualización parcial de Expense al transporte HTTP mediante el contexto controlado. Valida el identificador y los campos públicos, rechaza campos internos y delega la operación atómica en `updateExpense`, reutilizando las reglas existentes de estado, hogar, items, splits y `UPDATED_NOT_HYDRATED`.

El incremento cerrado `DELETE /api/expenses/{id}` adapta la eliminación de Expense al transporte HTTP mediante el contexto controlado. Rechaza parámetros de query, valida el identificador, delega en `deleteExpense` y conserva la semántica de la RPC existente, con respuesta `204` en éxito y errores HTTP sanitizados.
