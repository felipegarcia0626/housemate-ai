...
`app/api/expenses/[id]/route.ts` expone `GET`, `PATCH` y `DELETE /api/expenses/{id}`, obtiene el hogar mediante el contexto controlado, delega en `getExpense`, `updateExpense` o `deleteExpense` y proyecta el DTO público cuando corresponde.
